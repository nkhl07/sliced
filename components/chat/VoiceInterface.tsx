...
import { Mic, MicOff, PhoneOff, ChefHat, AlertTriangle, Wifi, Keyboard, Send, ShoppingBag, X } from 'lucide-react';
    ...
    <div className="flex h-[100dvh] overflow-hidden bg-[#FDFCF8]">
      <div className="flex flex-col flex-1 min-w-0">
        ...
        <motion.header
          ...
          className="flex items-center justify-between px-4 sm:px-8 py-4 sm:py-5 border-b border-stone-100 bg-white/80 backdrop-blur-sm"
        >
          <div className="flex items-center gap-3">
            <div className={`w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-gold/10 border flex items-center justify-center transition-all ${
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="font-serif font-black text-stone-900 text-base sm:text-lg leading-none">Sage</span>
              ...
              <p className="text-xs text-stone-400 mt-0.5 hidden sm:block">The Olive Branch · Voice Ordering</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <AnimatePresence>
              {isActive && (
                <motion.div
                  ...
                  className="hidden sm:flex items-center gap-2 bg-emerald-50 text-emerald-700 px-4 py-2 rounded-full text-xs font-bold border border-emerald-100"
                >
            ...
            <button
              ...
              className="md:hidden relative flex items-center justify-center w-9 h-9 bg-stone-50 border border-stone-200 rounded-xl text-stone-500"
            >
              <ShoppingBag size={17} />
            </button>
          </div>
        ...
        <motion.div
          ...
          className="px-4 sm:px-8 pb-6 sm:pb-8 pt-3 flex flex-col items-center gap-3 sm:gap-4"
        >
          {/* Waveform */}
          <AnimatePresence>
            {isActive && (
          ...
          <AnimatePresence mode="wait">
            ...
            {isActive || isConnecting ? (
              ...
              /* Text input mode */
              <motion.div key="text-input" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="w-full max-w-xl px-0">
        ...
        {mobileOrderOpen && (
          <>
            ...
            <motion.div
              ...
              className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white rounded-t-[32px] shadow-2xl max-h-[80vh] flex flex-col"
            >
              <div className="flex items-center justify-between px-6 pt-5 pb-3 border-b border-stone-100">
                <span className="font-serif font-black text-stone-900 text-lg">Your Order</span>
                <button onClick={() => setMobileOrderOpen(false)} className="w-8 h-8 flex items-center justify-center text-stone-400 hover:text-stone-700 rounded-lg">
                  <X size={18} />
                </button>
              </div>
